...
""" PART 0: BEFORE WE START ===================================================
    ...
    Look for the Kite indiciator in the bottom left corner of Spyder's status
...
# • Type "." and select the completion for "pyplot as plt".
# • Remember to press ctrl+space if autocompletions aren't on.
...
# 1d. Snippets
#
# Kite uses Spyder's native snippets functionality to help you complete
...
# them easily. You can jump between placeholders by pressing the tab key.
# Spyder enables snippets by default and you can turn them on/off by opening
# settings, clicking on "Completion and linting", and changing the "Enable code
# snippets" setting.

# TRY IT
# ------
# • Ensure that Spyder's "Enable code snippets" setting is enabled.
# • Put your cursor at the end of the line marked with "<--".
# • Type "t", and Kite will suggest calling "plot(<x>, <y>)".
# • Use Kite's completions and the tab key to fill out the function call. The
#   correct call should be "plt.plot(my_x, my_y)".
# • Remember to press ctrl+space if autocompletions aren't on.

import numpy as np

my_x = np.linspace(-2*np.pi, 2*np.pi)
my_y = np.sin(my_x)
